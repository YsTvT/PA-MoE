...
class TextDQNAgent(BaseAgent):
    ...
    def command_generation_act(self, observation_strings, task_desc_strings, previous_dynamics, random=False):
        with torch.no_grad():
            ...
            current_dynamics = torch.stack(current_dynamics, 0) if self.recurrent else None
            return chosen_actions, chosen_indices, current_dynamics
    ...
    def get_drqn_loss_admissible_commands(self):
        ...
        for step_no in range(self.rl_replay_sample_history_length):
            ...
            q_value = ez_gather_dim_1(action_scores, chosen_indices).squeeze(1)
            ...
            reward = reward + next_q_value * self.discount_gamma_game_reward
            loss = F.smooth_l1_loss(q_value, reward)
    ...
    def get_dqn_loss_beam_search_choice(self):
        ...
        with torch.no_grad():
            ...
            h_obs, obs_mask = self.encode(next_obs_list, use_model="target")
            h_td_t, td_mask_t = self.encode(task_list, use_model="target")
            aggregated_obs_representation = self.target_net.aggretate_information(h_obs, obs_mask, h_td_t, td_mask_t)
    ...
    def get_drqn_loss_beam_search_choice(self):
        ...
        for step_no in range(self.rl_replay_sample_history_length):
            ...
            with torch.no_grad():
                ...
                h_obs, obs_mask = self.encode(next_obs, use_model="target")
                aggregated_obs_representation = self.target_net.aggretate_information(h_obs, obs_mask, h_td_t, td_mask_t)
                averaged_representation = self.target_net.masked_mean(aggregated_obs_representation, obs_mask)
    ...
    def get_dqn_loss_command_generation(self):
        ...
        with torch.no_grad():
            ...
            next_aggregated_obs_representation = self.online_net.aggretate_information(next_h_obs, next_obs_mask, h_td, td_mask)
            ...
            for _ in range(self.max_target_length):
                ...
                input_target = to_pt(input_target, self.use_cuda)
                target_mask = compute_mask(input_target)
                pred = self.online_net.decode(input_target, target_mask, next_aggregated_obs_representation, next_obs_mask, None, next_input_obs)
                pred = to_np(pred[:, -1])
                pred = np.argmax(pred, -1)
                for b in range(batch_size):
                    new_stuff = [pred[b]] if eos[b] == 0 else []
            ...
            next_aggregated_obs_representation = self.target_net.aggretate_information(next_h_obs, next_obs_mask, next_h_td, next_td_mask)

            next_target_mask = compute_mask(next_input_target)
            next_pred = self.target_net.decode(next_input_target, next_target_mask, next_aggregated_obs_representation, next_obs_mask, None, next_input_obs)
            next_q_value = GetGenerationQValue(next_pred * next_target_mask.unsqueeze(-1), next_ground_truth, next_target_mask)
    ...
    def get_drqn_loss_command_generation(self):
        ...
        for step_no in range(self.rl_replay_sample_history_length):
            ...
            with torch.no_grad():
                ...
                next_h_obs, next_obs_mask = self.encode(next_observation_strings, use_model="online")
                next_aggregated_obs_representation = self.online_net.aggretate_information(next_h_obs, next_obs_mask, h_td, td_mask)
                next_averaged_representation = self.online_net.masked_mean(next_aggregated_obs_representation, next_obs_mask)
                ...
                for _ in range(self.max_target_length):
                    ...
                    input_target = to_pt(input_target, self.use_cuda)
                    target_mask = compute_mask(input_target)
                    pred = self.online_net.decode(input_target, target_mask, next_aggregated_obs_representation, next_obs_mask, next_dynamics, next_input_obs)
                    pred = to_np(pred[:, -1])
                    pred = np.argmax(pred, -1)
                    for b in range(batch_size):
                        new_stuff = [pred[b]] if eos[b] == 0 else []
                ...
                next_h_obs, next_obs_mask = self.encode(next_observation_strings, use_model="target")
                next_aggregated_obs_representation = self.target_net.aggretate_information(next_h_obs, next_obs_mask, h_td_t, td_mask_t)
                next_averaged_representation = self.target_net.masked_mean(next_aggregated_obs_representation, next_obs_mask)
                ...
                next_target_mask = compute_mask(next_input_target)
                next_pred = self.target_net.decode(next_input_target, next_target_mask, next_aggregated_obs_representation, next_obs_mask, next_dynamics, next_input_obs)
                next_q_value = GetGenerationQValue(next_pred * next_target_mask.unsqueeze(-1), next_ground_truth, next_target_mask)
