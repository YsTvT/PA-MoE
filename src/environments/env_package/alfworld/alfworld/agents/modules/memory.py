...
class PrioritizedReplayMemory(object):
    ...
    def get_batch(self, batch_size, multi_step=1):
        ...
        chosen_indices_list = np.array(chosen_indices_list)
    ...
    def get_batch_of_sequences(self, batch_size, sample_history_length):
        ...
        contains_first_step = random_number[0] < 0.05
        ...
        for i in range(sample_history_length):
            reward[i] = torch.stack(reward[i], 0)
            chosen_indices[i] = np.array(chosen_indices[i])
