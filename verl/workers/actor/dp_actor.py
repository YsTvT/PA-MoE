...
class DataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        if hasattr(self.actor_module, 'moe_actor') or hasattr(self.actor_module, 'expert_heads'):
            try:
                if hasattr(self, '_last_moe_output'):
                    from verl.moe.moe_wrapper import extract_moe_metrics
                    moe_metrics = extract_moe_metrics(self._last_moe_output)
                    append_to_dict(metrics, moe_metrics)
            except Exception as e:
                logger.warning(f"Failed to extract MoE metrics: {e}")
