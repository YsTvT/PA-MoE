...
class SGLangRollout(BaseRollout):
    ...
    def _verify_config(self, model_hf_config):
        ...
        assert self.config.max_model_len >= self.config.prompt_length + self.config.response_length, f
    ...
    def _req_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for req in sorted_output_req_list:
            ...
            assert len(req.input_ids) == len(req.attention_mask) == len(req.position_ids) == len(req.loss_mask), f
            error_message_lines = [
                f,
